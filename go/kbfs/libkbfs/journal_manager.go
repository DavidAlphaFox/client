...
func (j *JournalManager) getConflictIDForHandle(
	...
	j.log.CDebugf(nil, "Looking up conflict handle for %s", h.GetCanonicalPath())
	j.lock.RLock()
	...
	if !ok {
		j.log.CDebugf(nil, "Didn't find %s", h.GetCanonicalPath())
		return tlf.ID{}, false
	}

	j.log.CDebugf(nil, "Found conflict ID %s for %s", val.fakeTlfID, h.GetCanonicalPath())
...
func (j *JournalManager) getJournalsInConflictLocked(ctx context.Context) (
	...
	for key, val := range j.clearedConflictTlfs {
		...
		j.log.CDebugf(ctx, "Looking at conflict journal %s", fakeTlfID)
		...
		j.log.CDebugf(ctx, "Adding conflict journal %s", handle.GetCanonicalPath())
